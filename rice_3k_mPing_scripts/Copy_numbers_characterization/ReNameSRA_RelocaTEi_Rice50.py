...
def runjob(script, lines):
    #cmd = 'perl /rhome/cjinfeng/BigData/software/bin/qsub-pbs.pl --queue --maxjob 1 --lines %s --interval 120 --resource nodes=1:ppn=32,walltime=100:00:00 --convert no %s' %(lines, script)
    cmd = 'perl /rhome/cjinfeng/BigData/software/bin/qsub-slurm.pl --maxjob 30 --lines %s --interval 120 --task 24 --mem 40G --time 20:00:00 --convert no %s' %(lines, script)
    #print cmd 
    os.system(cmd)
...
def main():
    ...
    if not os.path.exists(project):
        os.mkdir(project)
    print project
    read_dirs = glob.glob('%s/*_*' %(os.path.abspath(args.input)))
    ...
    for read_dir in sorted(read_dirs):
        ...
        if not os.path.exists(outdir):
        #if 1:
            relocaTE = '%s --te_fasta %s --genome_fasta %s --fq_dir %s --outdir %s --reference_ins %s --step 1234567 --mismatch 2 --aligner blat --cpu 32 --verbose 3 --split --run > %s.log 2> %s.std' %(RelocaTE, Repeat, Reference, read_dir, outdir, existingTE, outdir, outdir)
            ...
            print 'output directory %s already exists: check if you have result or delete this fold' %(outdir)
    ofile.close()
