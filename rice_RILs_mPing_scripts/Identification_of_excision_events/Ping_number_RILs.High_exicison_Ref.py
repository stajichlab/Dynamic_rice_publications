...
def read_csv(infile, ping_code, locus_gt, outdir):
        ...
        for line in filehd:
            ...
            if len(line) > 2: 
                ...
                if unit[0].startswith(r'Chr'):
                    ...
                    locus = unit[0]
                    unit[0] = '%s:%s' %(locus, locus_gt[locus])
                    ...
                    if (locus_gt[locus] == 'Ref' and unit[1] == 'NB') or locus_gt[locus] == 'Shared':
                        #excision
...
def readgff(infile):
    ...
    with open (infile, 'r') as filehd:
        for line in filehd:
            ...
            if len(line) > 2: 
                unit  = re.split(r'\t',line)
                locus = '%s:%s-%s' %(unit[0], unit[3], unit[4]) 
                if pt.search(unit[8]):
                    data[locus] = 'Shared'
                else:
                    data[locus] = 'Ref'
