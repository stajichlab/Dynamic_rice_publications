...
my $dbSNP  = '/bigdata/wesslerlab/shared/Rice/HEG4/dbSNP/dbSNPs_VQSR.vcf';
my $GENOME = '/bigdata/wesslerlab/shared/Rice/Genome/index/MSU_r7.corrected.fa';
...
my $qsub_q = defined $QUEUE_Q ? "-p $QUEUE_Q" : '-p intel';
my $qsub_l = defined $QUEUE_L ? "-l $QUEUE_L" : '--nodes=1 --ntasks=8 --mem=60G --time=40:00:00 --workdir=./';
...
genotype.todo.txt: \$(FINAL_RENAME)
	for i in `cat \$^` ; do\\
	 ...
	 JOB=`sbatch $qsub_q $qsub_l \$(HOME)/run.genotype.\$\$SHORT.sh`;\\
	 echo \"`date` \$\$JOB run.genotype.\$\$SHORT.sh\" >> \$\@;\\
	done;\\
        printf \"\\nThe following jobs have been submitted to the queue:\\n\";\\
	cat \$\@

\$(FINAL_RENAME): \$(SCRIPTS)/rename_files_gz.pl \$(RENAME_DIR) \$(HOME) $id_file
	\$^ > \$\@
