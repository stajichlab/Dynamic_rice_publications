...
$opt{read} ||= 20000000; ##950000 reads will make 0.5X of 372 Mb of rice genome. 950000x100x2/372000000 ~= 0.5, 20M = 10 X
...
for (my $i=0;$i<@fq;$i++){
   ...
   if ($fq[$i]=~/(.*\/RIL(\d+)\_\d+\_*\w*)_p1\.fq/){
      ...
      unless (-e "$file1" and -e "$file2"){
         ...
         my $cmd ="python /rhome/cjinfeng/BigData/00.RD/RILs/QTL_pipe/bin/scripts/fastq/prefastq.py $fq[$i] $fq2 $file1 $file2 $opt{read}";
         #print OUT "$cmd1\n$cmd2\n";
         print OUT "$cmd\n";
      }
      $rils{$name}=1;
