...
use Data::Dumper;
my %pairs;
## for CORNELL RUNS and UCR RUNS now!!
...
while (my $line = <IDFILE>){
  ...
  $sample =~ s/_pair\d//; #incase the pair info did not get removed in the input file, do it now 
  $rename{$sample}{code}=$code;
  $rename{$sample}{adaptor}=$adaptor;
  $rename{$sample}{STRAIN}=$RIL;
...
foreach my $file (@files){
  ...
  if($file =~ /(flowcell\d+_lane\d+)_pair(\d)_([ATCG]{6})\.fastq\.gz/){
    ## UCR files
    $sample = $1 . "_" . $3;
    ($barcode,$pair) = ($3,$2);
    print "UCR\n" if $test;
  }
  elsif ($file =~ /(\d+_\d+_.+_([ATCG]{6}))_(R\d)\.fastq\.gz/){
    ## cornell files
    ($sample,$barcode,$pair) = $file =~ /(\d+_\d+_.+_([ATCG]{6}))_(R\d)\.fastq.gz/;
    print "CORNELL\n" if $test;
  }
  ...
  if (exists $rename{$sample}){
    my $strain = $rename{$sample}{STRAIN};
    if ($strain =~ /^\d+\$/){
      ## STRAINS with just digits are RILs
      $strain = "RIL" . $rename{$sample}{STRAIN};
    }
